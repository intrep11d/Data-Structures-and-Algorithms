...
void displayArrListStud(arrListStud A)
{
   ...
   //Write your code here.   
    for (x = 0; x < A.numStuds; x++) {
        printf("%-10s %-10s %-10s %-10s %-10d\n", A.studs[x].idNum, A.studs[x].name.fName, A.studs[x].name.lName, getProgram(A.studs[x].info), yearLevelHash(A.studs[x].info));
    }
...
{
     //Write your code here
    int x, programNdx, yearNdx, checker;

    for (x = 0; x < SL.numStuds; x++) {
        programNdx = programHash(SL.studs[x].info);
        yearNdx = yearLevelHash(SL.studs[x].info) - 1;
        // printf("%d\n", yearNdx);
        checker = insertStudLL(&D[programNdx].programStuds[yearNdx], SL.studs[x]);
        if (checker != 1) {
            printf("\nStudent %s already exists", D[programNdx].programStuds[yearNdx]->stud.idNum);
        } else {
...
studSet* initStudSet(void)
{
     ...
     studSet* studentSet;
     int x;
     studentSet = (studSet*)malloc(sizeof(sNode) * NUMPROGRAMS);
     for (x = 0; x < NUMPROGRAMS; x++) {
